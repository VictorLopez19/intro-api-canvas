...
function drawArcs(ctx) {
    ...
    for (let i = 0; i < 4; i++) {
        for (let j = 0; j < 3; j++) {
            ...
            const endAngle = Math.PI + (Math.PI * j) / 2; // Ángulo final
            const counterclockwise = i % 2 !== 0; // Dirección del arco (sentido contrario a las agujas del reloj)
...
canvases.forEach((canvas, index) => {
    if (canvas && canvas.getContext) {
        ...
        // Llamar a la función correspondiente según el índice
